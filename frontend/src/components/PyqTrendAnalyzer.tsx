...
import { cleanTopicTitle } from '../services/aiService';
...
export const PyqTrendAnalyzer: React.FC<PyqTrendAnalyzerProps> = ({
  ...
  useEffect(() => {
    if (studyDoc) {
      setIsAnalyzing(true);
      const pyqDocToUse = pyqDoc || {
        id: 'pyq-auto',
        fileName: 'Auto-Extracted Exam Set',
        title: studyDoc.title,
        pages: studyDoc.pages,
        text: studyDoc.text,
        uploadedAt: new Date().toISOString()
      };
      comparePyqWithStudyMaterial(studyDoc, pyqDocToUse)
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-4">
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="px-3 py-1.5 rounded-lg bg-purple-500/10 text-purple-300 border border-purple-500/30 font-medium">
              PYQ: {pyqDoc ? pyqDoc.title : 'Extracted from Study PDF'}
            </span>
          </div>
        </div>
              ...
              <div key={idx} className="glass-card rounded-2xl p-6 border border-slate-800 space-y-5">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <h4 className="text-base font-bold text-slate-100">{cleanTopicTitle(item.topic)}</h4>
                  </div>
                    ...
                    {item.predictedQuestions.map((pq, pidx) => (
                      <div key={pidx} className="bg-slate-950 p-3.5 rounded-xl border border-slate-900 space-y-1.5">
                        ...
                        <p className="text-xs text-slate-200 font-medium">
                          {cleanTopicTitle(pq.questionText)}
                        </p>
                      </div>
                    ))}
                  </div>
