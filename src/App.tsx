...
export function App() {
  ...
  const [activeDoc, setActiveDoc] = useState<PdfDocument | null>(SAMPLE_STUDY_DOC);
  const [pyqDoc, setPyqDoc] = useState<PyqDocument | null>(SAMPLE_PYQ_DOC);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Document Upload Hero (Shown when on pdf-viewer or no doc loaded) */}
        {!activeDoc ? (
          <DocumentUpload
            activeDoc={activeDoc}
      ...
      <footer className="border-t border-slate-900 bg-slate-950/80 py-6 text-center text-xs text-slate-500 no-print">
        <p>MindCraft AI PDF Reader, Explainer & Question Generator • Powered by Google Gemini AI</p>
      </footer>
