...
import { generateImportantQuestions } from '../services/aiService';
...
export const ImportantQuestions: React.FC<ImportantQuestionsProps> = ({
  ...
  useEffect(() => {
    if (!document) return;
    setIsLoading(true);
    generateImportantQuestions(document, language)
  ...
  const handleExportTxt = () => {
    ...
    filteredQuestions.forEach((q, idx) => {
      txt += `Q${idx + 1}. [${q.importance} - ${q.marks} MARKS] (${q.topic})\n`;
      txt += `${q.questionText}\n\n`;
      txt += `MODEL ANSWER:\n${q.modelAnswer}\n`;
      txt += `------------------------------------------------------------------------\n\n`;
    });
    ...
    <div className="space-y-6 animate-fadeIn max-w-5xl mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-6 rounded-2xl glass-card border border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Categorized by exam weightage (2, 5 & 10 marks) with complete model answers
          </p>
        </div>

        {/* Export Buttons & Filter Chips */}
      ...
      {isLoading ? (
        <div className="py-16 text-center text-xs text-indigo-300 animate-pulse space-y-3">
          ...
          <p>Extracting high-priority exam questions & generating model answers...</p>
        </div>
      ) : (
