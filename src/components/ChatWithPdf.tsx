...
export const ChatWithPdf: React.FC<ChatWithPdfProps> = ({
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      sender: 'ai',
      text: document 
        ? `Hello! I've read and analyzed **${document.title}**. You can ask me any question about the concepts, formulas, algorithms, or definitions in this PDF.`
  ...
  const presetPrompts = [
    "Explain the ID3 Algorithm step-by-step",
    "State Bayes Theorem & Naive assumption",
    "What is the Bias-Variance Tradeoff?",
    "Define Precision, Recall, and F1-Score",
    "List top 2-mark questions from Chapter 1"
  ];
          ...
          {isLoading && (
            <div className="flex gap-3 items-center text-xs text-indigo-300 animate-pulse">
              ...
              <div className="p-3 rounded-2xl bg-slate-900 border border-slate-800 flex items-center gap-2">
                ...
                Analyzing PDF text & generating explanation...
              </div>
            </div>
          )}
        </div>
